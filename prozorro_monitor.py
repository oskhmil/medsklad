...
EDRPOU = "44496574"
# 336  — фармацевтична продукція (у додатку показується як «Ліки»)
# 3314 — медичні матеріали
# 3317 — анестезія та реанімація
# 3319 — медичні вироби різні (часто «збірний» код для витратки)
CPV_PREFIX = ("336", "3314", "3317", "3319")
# Усе, що не 336*, у додатку показується як 🧰 ВМП («Матеріали»).
MAT_PREFIX = ("3314", "3317", "3319")
...
BUYER_FIELD = "buyer"
CPV_ROOT = "33600000-6"
# Розділ 336 (фармацевтична продукція) + групи 3314 (медичні матеріали),
# 3317 (анестезія та реанімація) і 3319 (медичні вироби різні).
...
CPV_CHILDREN = [
    ...
    "33149000-6",
    # 3317 — анестезія та реанімація
    "33170000-2", "33171000-9", "33171100-0", "33171110-3", "33171200-1",
    "33171210-4", "33172000-6", "33172100-7", "33172200-8",
    # 3319 — медичні вироби різні
    "33190000-8", "33191000-5", "33191100-6", "33191110-9", "33192000-2",
    "33192100-3", "33192200-4", "33192300-5", "33192400-6", "33192500-7",
    "33193000-9", "33194000-6", "33194100-7", "33194110-0", "33194120-3",
    "33194200-8", "33194210-1", "33194220-4", "33195000-3", "33196000-0",
    "33197000-7", "33198000-4", "33199000-1",
]
...
def discover_tender_ids():
    ...
    for code, expected in sorted(good, key=lambda x: -x[1]):
        group = "Матеріали" if code.startswith(MAT_PREFIX) else "Ліки"
        page = 1
        got = 0
        retries = 0
        while page <= 60:
